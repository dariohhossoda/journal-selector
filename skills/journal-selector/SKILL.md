...
Write the scores into a JSON file shaped like `templates/candidates.example.json` — one object per candidate, one entry per criterion, each with a `note` recording the source and the date. The note field is not optional decoration; it is the audit trail, and the renderer emits it as an evidence list.
...
### Step 6 — Verify the winner's scope
